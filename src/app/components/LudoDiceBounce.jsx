...
const LudoDiceBounce = () => {
  ...
  const roll = async () => {
    ...
    await controls.start({
      y: [0, -80, 0, -30, 0, -10, 0],
      rotateX: [0, 200, 320, 360 + x],
      rotateY: [0, 160, 280, 360 + y],
      transition: {
        duration: 1.2,
        ease: "easeOut",
  ...
  return (
    <div className="w-full" style={{ width: 360 }}>
      <h2 className="text-center mt-2">Dice — Bounce</h2>
      <div className="flex flex-col items-center justify-center py-6">
        <div className="relative" style={{ perspective: 900 }}>
          ...
          >
            {/* faces identical to other components */}
        ...
        <div className="mt-6">
          <motion.button onClick={roll} disabled={isRolling} className={`px-6 py-3 rounded-xl font-bold text-lg transition-all duration-300 ${isRolling ? "bg-gray-500 cursor-not-allowed text-white" : "bg-rose-600 hover:bg-rose-700 text-white shadow-lg hover:shadow-xl"}`} whileHover={!isRolling ? { scale: 1.05 } : {}} whileTap={!isRolling ? { scale: 0.97 } : {}}>
            {isRolling ? "Rolling..." : "Roll (Bounce)"}
          </motion.button>
        </div>
